...
let tools = [
  {
    ...
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" height="20px" width="20px"><!-- Font Awesome Free 5.15.4 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) --><path d="M256 32C114.6 32 0 125.1 0 240c0 49.6 21.4 95 57 130.7C44.5 421.1 2.7 466 2.2 466.5c-2.2 2.3-2.8 5.7-1.5 8.7S4.8 480 8 480c66.3 0 116-31.8 140.6-51.4 32.7 12.3 69 19.4 107.4 19.4 141.4 0 256-93.1 256-208S397.4 32 256 32z"/></svg>',
    click: showHitokoto
  },
  {
    className: 'custom-fa-user-circle',
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512" height="20px" width="20px"><!-- Font Awesome Free 5.15.4 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) --><path d="M248 8C111 8 0 119 0 256s111 248 248 248 248-111 248-248S385 8 248 8zm0 96c48.6 0 88 39.4 88 88s-39.4 88-88 88-88-39.4-88-88 39.4-88 88-88zm0 344c-58.7 0-111.3-26.6-146.5-68.2 18.8-35.4 55.6-59.8 98.5-59.8 2.4 0 4.8.4 7.1 1.1 13 4.2 26.6 6.9 40.9 6.9 14.3 0 28-2.7 40.9-6.9 2.3-.7 4.7-1.1 7.1-1.1 42.9 0 79.7 24.4 98.5 59.8C359.3 421.4 306.7 448 248 448z"/></svg>',
    click: loadRandModel
  },
  {
...
function loadModel() {
  ...
  if (window.loadlive2d && apiPath && modelPath && modelTexturesId) {
    // @ts-ignore
    window.loadlive2d(customId, `${apiPath}/${modelPath}/${modelTexturesId}.json`)
...
function loadRandModel() {
  http({
    ...
    success: (data: any) => {
      const models = data.filter(({ modelPath: _modelPath }: any) => modelPath !== _modelPath)
      const { modelPath: _modelPath, modelIntroduce } =
        models[Math.floor(Math.random() * models.length)]
      if (modelPath !== _modelPath) {
        setModel([modelPath, modelTexturesId])
...
function loadRandTextures(isAfterRandModel = false) {
  http({
    ...
    success: (data: any) => {
      ...
      const _modelTexturesId = modelTexturesIds[Math.floor(Math.random() * modelTexturesIds.length)]
      if (modelTexturesId !== _modelTexturesId) {
        setModel([modelPath, _modelTexturesId])
        modelTexturesId = _modelTexturesId
      }
      loadModel()
      if (!isAfterRandModel) {
...
<template>
  ...
  >
    <div
      ...
      :class="[isTipBottom ? 'vue3-live2d-tip-on-bottom' : 'vue3-live2d-tip-on-top']"
    />
    <canvas
    ...
    <div
      ...
      :class="[isLeft ? 'vue3-live2d-toggle-on-left' : 'vue3-live2d-toggle-on-right']"
    >
      <span>看板娘</span>
    </div>
  </div>
